    ...
    <div className="card p-5 space-y-3">
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          {absence.comentario_trabajador && (
            <p className="text-xs mt-1.5 p-2 rounded-lg italic" style={{ backgroundColor: 'var(--surface)', color: 'var(--text-secondary)' }}>
              {absence.comentario_trabajador}
            </p>
          )}
        </div>
      ...
      {absence.estado === 'pendiente' && (
        <div className="flex gap-2 pt-1">
          <button
            className="flex-1 flex items-center justify-center gap-1.5 text-white text-xs font-medium rounded-xl py-2 transition-colors"
      ...
      {absence.comentario_rrhh && (
        <p className="text-xs rounded-lg p-2" style={{
          backgroundColor: absence.estado === 'denegada' ? '#fee2e2' : '#dcfce7',
          color: absence.estado === 'denegada' ? '#b91c1c' : '#15803d',
        }}>
          <span className="font-semibold">RRHH: </span>{absence.comentario_rrhh}
        </p>
      )}
    </div>
  )
...
export default function HRAbsences() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>Gestión de ausencias</h1>
        <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>Aprueba o deniega las solicitudes de ausencia.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12" style={{ color: 'var(--text-muted)' }}>No hay solicitudes para los filtros seleccionados</div>
      ) : (
        <div className="space-y-3">
          <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{absences.length} solicitudes</p>
        ...
        >
          <form onSubmit={handleReview} className="space-y-4">
            <div className="rounded-xl p-3 text-sm" style={{
              backgroundColor: reviewing.action === 'aprobada' ? '#dcfce7' : '#fee2e2',
              color: reviewing.action === 'aprobada' ? '#15803d' : '#b91c1c',
            }}>
            ...
            <div>
              <label className="label">
                Comentario {reviewing.action === 'denegada' && <span style={{ color: 'var(--danger)' }}>*</span>}
              </label>
              <textarea
            ...
            {submitError && (
              <div className="flex items-center gap-2 rounded-xl px-3 py-2 text-xs" style={{ backgroundColor: '#fee2e2', color: '#b91c1c', border: '1px solid #fecaca' }}>
            ...
            <div className="flex gap-3">
              ...
              <button
                type="submit"
                disabled={submitting}
                className="flex-1 flex items-center justify-center gap-2 font-medium rounded-xl py-2.5 text-white"
                style={{ backgroundColor: reviewing.action === 'aprobada' ? '#16a34a' : '#dc2626' }}
              >
                {submitting
