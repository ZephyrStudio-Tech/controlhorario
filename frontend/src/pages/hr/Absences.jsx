...
export default function HRAbsences() {
  ...
  const [reviewing, setReviewing] = useState(null) // { absence, action }
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          ...
          {absences.map((a) => (
            <AbsenceCard key={a.id} absence={a} onReview={openReview} />
          ))}
        </div>
      )}
          ...
          <form onSubmit={handleReview} className="space-y-4">
            <div className={`rounded-xl p-3 text-sm ${reviewing.action === 'aprobada' ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'}`}>
              <p className="font-semibold">
                {reviewing.absence.user?.nombre} {reviewing.absence.user?.apellidos}
              </p>
              <p className="text-xs mt-0.5">
                {TIPOS[reviewing.absence.tipo]} · {reviewing.absence.fecha_inicio} → {reviewing.absence.fecha_fin}
              </p>
            </div>
            <div>
            ...
            <div className="flex gap-3">
              <button type="button" className="btn-secondary flex-1" onClick={() => setReviewing(null)}>
                Cancelar
              </button>
              <button
                type="submit"
